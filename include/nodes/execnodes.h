 ...
 *		hj_NullOuterTupleSlot	prepared null tuple for right/full inner joins
...
#else
/* modified: 为结构体添加字段以支持双向探测 */
typedef struct HashJoinState
{
	JoinState	js;				/* its first field is NodeTag */
	ExprState  *hashclauses;
	List	   *hj_OuterHashKeys;	/* list of ExprState nodes */
	List	   *hj_InnerHashKeys;	/* modified: 增加表达式节点的哈希链表 */
	List	   *hj_HashOperators;	/* list of operator OIDs */
	List	   *hj_Collations;

	/* modified:扩展内部和外部哈希表字段 */
	HashJoinTable hj_InnerHashTable;
	HashJoinTable hj_OuterHashTable;

	/* modified:扩展当前内部和外部哈希值字段 */
	...
	HashJoinTuple hj_CurOuterTuple;

	/* modified:扩展内部和外部元组的槽字段*/
	TupleTableSlot *hj_OuterTupleSlot;
	...
	/* modified:扩展判断内部和外部元组是否为空的布尔值字段 */
	bool		hj_OuterNotEmpty;
	bool		hj_InnerNotEmpty;

	/* modified:扩展判断是否取完内部和外部元组的布尔值字段*/
	bool 	    hj_OuterIsExhausted;
	bool        hj_InnerIsExhausted;

 	/* modified:增加判断是否是正在匹配内部关系哈希表的布尔值字段 */
	bool        IsFetchInnerNode; 
