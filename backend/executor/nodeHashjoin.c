...
ExecInitHashJoin(HashJoin *node, EState *estate, int eflags)
{
	...
	hashjoin_state->js.jointype = node->join.jointype;
    ...
    /* modified:初始化outer和inner子节点 */
	/* explain: ExecInitNode:初始化节点 */
	Hash *outer_hashnode,*inner_hashnode;
    outer_hashnode = (Hash *) outerPlan(node);
	inner_hashnode = (Hash *) innerPlan(node);
	outerPlanState(hashjoin_state) = ExecInitNode((Plan *) outer_hashnode, estate, eflags);
	innerPlanState(hashjoin_state) = ExecInitNode((Plan *) inner_hashnode, estate, eflags);
	
    
    /* modified:初始化结果槽的类型、操作和投影 */
    /* explain: ExecGetResultType:获取结果类型 ExecGetResultSlotOps:获取结果槽操作  
		        ExecInitExtraTupleSlot:获取额外的元组槽用来存放查询操作的中间结果
    */
    TupleDesc outer_Desc,inner_Desc;
	TupleTableSlotOps *outer_ops,*inner_ops;
    outer_Desc = ExecGetResultType(outerPlanState(hashjoin_state));
	inner_Desc = ExecGetResultType(innerPlanState(hashjoin_state));
    outer_ops = ExecGetResultSlotOps(outerPlanState(hashjoin_state), NULL);
	hashjoin_state->hj_OuterTupleSlot = ExecInitExtraTupleSlot(estate, outer_Desc, outer_ops);
	inner_ops = outer_ops;
	...
	switch (node->join.jointype)
	{
		...
		case JOIN_SEMI:
			break;
		case JOIN_LEFT:
			hashjoin_state->hj_NullInnerTupleSlot =
				ExecInitNullTupleSlot(estate, inner_Desc, &TTSOpsVirtual);
			break;
		case JOIN_ANTI:
			hashjoin_state->hj_NullInnerTupleSlot =
				ExecInitNullTupleSlot(estate, inner_Desc, &TTSOpsVirtual);
			break;
		case JOIN_RIGHT:
			hashjoin_state->hj_NullOuterTupleSlot =
				ExecInitNullTupleSlot(estate, outer_Desc, &TTSOpsVirtual);
			break;
		case JOIN_FULL:
			hashjoin_state->hj_NullOuterTupleSlot =
				ExecInitNullTupleSlot(estate, outer_Desc, &TTSOpsVirtual);
...
*/
	...
	HashState* hashstate;
	TupleTableSlot *slot;
	hashstate = (HashState *) innerPlanState(hashjoin_state);
	slot = hashstate->ps.ps_ResultTupleSlot;
	hashjoin_state->hj_InnerHashTupleSlot = slot;
	hashstate = (HashState *) outerPlanState(hashjoin_state);
	slot = hashstate->ps.ps_ResultTupleSlot;
	hashjoin_state->hj_OuterHashTupleSlot = slot;	
	hashjoin_state->hj_CurInnerTuple = NULL;
	...
	hashjoin_state->hj_InnerCurHashValue = 0;
	...
	List* operators;
	ListCell *cell;
	left_clauses = (List*)NULL;
	right_clauses = (List*)NULL;
	operators = (List*)NULL;
	for(cell = list_head(node->hashclauses);cell!=(ListCell*)NULL;cell = cell->next)
	{
		OpExpr *express = (OpExpr *)lfirst(cell);
		left_clauses = lappend(left_clauses, ExecInitExpr(linitial(express->args),(PlanState*)hashjoin_state));
		right_clauses = lappend(right_clauses, ExecInitExpr(lsecond(express->args),(PlanState*)hashjoin_state));
		operators = lappend_oid(operators, express->opno);
	}
	((HashState*)innerPlanState(hashjoin_state))->hashkeys = right_clauses;
	((HashState*)outerPlanState(hashjoin_state))->hashkeys = left_clauses;
	...
	hashjoin_state->hj_OuterIsExhausted = false;
	hashjoin_state->hj_InnerIsExhausted = false;
	hashjoin_state->hj_Matched = false;
	hashjoin_state->hj_OuterNotEmpty = false;
	hashjoin_state->hj_InnerNotEmpty = false;
	hashjoin_state->IsFetchInnerNode = true;
    hashjoin_state->hj_JoinState = HJ_BUILD_HASHTABLE;
